...
public class emit {
  ...
  public static String src_dir_name = "";
  ...
  protected static void emit_action_code(PrintWriter out, production start_prod)
      ...
      for (Enumeration p = production.all(); p.hasMoreElements(); )
	{
	  ...
	  /* make the variable RESULT which will point to the new Symbol (see
	     below) and be changed by action code
	     6/13/96 frankf */
	  ...
	  /* if there is just one symbol on the rhs of the action, and no
	     action code was specified, assign the rhs to the result.
	     01/21/99 jhyde */
	  if (!prod._userAction &&
	      prod.rhs_length() == 1 &&
	      prod.rhs(0) instanceof symbol_part) {
		  out.println("\t\t// Default action for single-symbol production");
	      out.println(
			  "\t\tRESULT = (" +
			  ((symbol_part) prod.rhs(0)).the_symbol().stack_type() +
			  ")((" + "java_cup.runtime.Symbol) " + emit.pre("stack") +
			  ".elementAt(" + emit.pre("top") + ")).value;");
	  }
