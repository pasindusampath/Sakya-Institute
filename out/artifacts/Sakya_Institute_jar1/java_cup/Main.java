 ...
 *   <dt> -srcdir dir
 *   <dd> specify the root directory of the java tyee [default the current directory]
 *   <dt> -interface
...
public class Main {
  ...
  /** Name of input file (or "Standard input"). */
  protected static String inFileName;

  /** Name of output file which holds parser class. */
  protected static String classFileName;

  /** Name of output file which holds symbols class. */
  protected static String symbolsFileName;
  ...
  /** Only generate output files if the specification is newer. **/
  protected static boolean newer = false;
  ...
  static void main(String argv[], String _inFileName)
    throws internal_error, java.io.IOException, java.lang.Exception
    {
	  ...
	  if (!compute_file_names(_inFileName)) {
		  if (print_progress) {
...
"    -newer         generate only if the specification is newer\n"
      );
      System.exit(1);
    }
  ...
  protected static void parse_args(String argv[])
      ...
      for (i=0; i<len; i++)
	{
	  ...
	  else if (argv[i].equals("-srcdir"))
	    {
	      /* must have an arg */
	      if (++i >= len || argv[i].startsWith("-") ||
				argv[i].endsWith(".cup"))
		usage("-srcdir must have a name argument");

	      /* record the name */
	      emit.src_dir_name = argv[i];
	    }
	  ...
	  /* jhyde 02-Apr-2002 */
	  else if (argv[i].equals("-newer"))        newer = true;
  ...
  /**
   * Figures out the names of the input and output files, checks their time-
   * stamps, and returns whether to go ahead.
   */
  protected static boolean compute_file_names(String _inFileName)
    {
      inFileName = _inFileName; // may be null
      classFileName = classToFileName(emit.package_name, emit.parser_class_name);
      symbolsFileName = classToFileName(emit.package_name, emit.symbol_const_class_name);
      if (newer && inFileName != null) {
	File inFile = new File(inFileName),
	    classFile = new File(classFileName),
	    symbolsFile = new File(symbolsFileName);
	if (!classFile.exists() ||
	    !symbolsFile.exists()) {
	  return true;
	}
	long inStamp = inFile.lastModified(),
	    classStamp = classFile.lastModified(),
	    symbolsStamp = symbolsFile.lastModified();
	if (inStamp > classStamp ||
	    inStamp > symbolsStamp) {
	  return true;
	}
	return false;
      } else {
  ...
  protected static void open_input_file()
    {
      if (print_progress) {
	...
	try {
	  ...
	  System.err.println("Can't open \"" + inFileName + "\" for input");
	  System.exit(4);
	}
      }

      if (print_progress) {
	System.err.println("Parsing specification from " + inFileName + "...");
      }
  ...
  protected static void open_files()
    {
      /* parser class */
      File fil = new File(classFileName);
      try {
        parser_class_file = new PrintWriter(
		 new BufferedOutputStream(new FileOutputStream(fil), 4096));
      } catch(Exception e) {
	System.err.println("Can't open \"" + classFileName + "\" for output");
	System.exit(3);
      }

      /* symbol constants class */
      fil = new File(symbolsFileName);
      try {
        symbol_class_file = new PrintWriter(
		 new BufferedOutputStream(new FileOutputStream(fil), 4096));
      } catch(Exception e) {
	System.err.println("Can't open \"" + symbolsFileName + "\" for output");
	System.exit(4);
      }
    }
  ...
  /*-----------------------------------------------------------*/
     ...
     **/
    static String classToFileName(String packageName, String className)
    {
	char fileSep = System.getProperty("file.separator").charAt(0); // e.g. '/'
	String s = emit.src_dir_name;
	if (s == null) {
	    s = "";
	} else if (s.equals("")) {
	} else {
	...
	return s + className.replace('.', fileSep) + ".java";
    }

    /**
     * Returns the full class name which lives in a java file.
     **/
    static String fileNameToClass(String fileName)
    {
	if (fileName.endsWith(".cup")) {
	    fileName = fileName.substring(
		0, fileName.length() - ".cup".length());
	}
	if (fileName.endsWith(".java")) {
	    fileName = fileName.substring(
		0, fileName.length() - ".java".length());
	}
	char fileSep = System.getProperty("file.separator").charAt(0); // e.g. '/'
